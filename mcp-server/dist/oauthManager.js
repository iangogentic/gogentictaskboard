...
export class OAuthManager {
    ...
    async ensureSession() {
        ...
        if (existing && new Date(existing.tokenExpiresAt) > new Date(Date.now() + 60000)) {
            return existing;
        }
    ...
    async getClient() {
        if (!this.clientPromise) {
            ...
            this.clientPromise = Promise.resolve(new issuer.Client({
    ...
    async exchangeWithNext(idToken) {
        const response = await axios.post(`${env.baseUrl}/api/mcp/auth`, {
            idToken,
        }, {
