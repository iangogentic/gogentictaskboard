...
export async function PATCH(
  ...
  try {
    ...
    if (oldProject.status !== status) {
      updates.push({
        projectId: id,
        userId: pmId, // Using PM as the user making the change
        content: `Project status changed from ${oldProject.status.replace('_', ' ')} to ${status.replace('_', ' ')}`,
    ...
    if (oldProject.pmId !== pmId) {
      updates.push({
        projectId: id,
        userId: pmId,
        content: `Project manager changed from ${oldProject.pm.name} to ${updatedProject.pm.name}`,
        isAutoGenerated: true,
      })
    }
