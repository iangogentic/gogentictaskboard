  ...
  try {
    ...
    const portfolioStats = portfolios.map((portfolio) => {
      ...
      const blockedCount = projects.filter(
        (p) => p.status === "Blocked" || p.status === "BLOCKED"
      ).length;
      ...
      const avgHealth =
        projects.length > 0
          ? Math.round(
              (healthyProjects * 100 + warningProjects * 60) / projects.length
            )
          : 100;
    ...
    const needsAttention = needsAttentionProjects.map((project) => {
      ...
      if (project.status === "Blocked" || project.status === "BLOCKED") {
        ...
        project.targetDelivery <= new Date(Date.now() + 3 * 24 * 60 * 60 * 1000)
      ) {
        const daysUntilDue = Math.ceil(
          (project.targetDelivery.getTime() - Date.now()) /
            (24 * 60 * 60 * 1000)
        );
