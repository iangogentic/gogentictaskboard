    ...
    if (savedTemplate) {
      ...
      setFormData(prev => ({
        ...
        targetDelivery: parsed.estimatedDuration ? 
          new Date(Date.now() + parsed.estimatedDuration * 24 * 60 * 60 * 1000).toISOString().split('T')[0] : ''
      }))
    ...
    try {
      const response = await fetch('/api/projects', {
        ...
        body: JSON.stringify({
          ...
          targetDelivery: formData.targetDelivery ? new Date(formData.targetDelivery) : null,
        }),
      })
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, targetDelivery: e.target.value }))}
                className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
          </div>
